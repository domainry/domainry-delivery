...
// one delivery. DeliveryUnits are the only development lifecycle.
func NewDeliveryRun(product Product, featureID string, featureRevision uint64, spec DeliveryRunSpec, actor Actor, now time.Time) (DeliveryRun, error) {
	...
	source := FeatureRevisionRef{
		...
		ProductRevision: featureRevisionValue.BaselineProductRevision,
	...
	return DeliveryRun{
		...
		Product: ProductSnapshot{ID: product.ID, Name: product.Name, Code: product.Code, ProductRevision: featureRevisionValue.BaselineProductRevision},
