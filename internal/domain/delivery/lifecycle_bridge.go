...
func StartDelivery(product *Product, deliveryRunID string, command Command, now time.Time) (DeliveryRun, error) {
	if command.Type != commandFeatureDelivery {
...
func InstallDeliveryRun(product *Product, run *DeliveryRun, actor Actor, now time.Time) error {
	...
	if len(run.DeliveryUnits) > 0 {
		return installV3DeliveryRun(product, run, feature, now)
	}
	...
	if liveRelease.ProductRevision != executable.TargetRevision || liveRelease.ProductRevisionRef != executable.EvidenceRef || liveRelease.CodeRevision != executable.CodeRevision {
	...
	product.Revisions = append(product.Revisions, ProductRevision{
		...
		ArtifactRef: liveRelease.ArtifactRef, DefinitionRef: executable.EvidenceRef,
...
func installV3DeliveryRun(product *Product, run *DeliveryRun, feature *Feature, now time.Time) error {
	revision, ok := v3VerificationRevision(run)
	if !ok || !allQualityPass(run, revision) || !allV3AcceptancePass(run, revision) {
		return Invalid("v3_release_gate_failed", "The live V3 DeliveryRun is not bound to complete Journey, QA, and acceptance evidence.")
	}
	var liveRelease *Release
	for index := range run.Releases {
		if run.Releases[index].Status == ReleaseLive {
	...
	if liveRelease == nil || liveRelease.CodeRevision != revision || liveRelease.DeploymentAttempt == nil || strings.TrimSpace(liveRelease.DeploymentAttempt.ReceiptRef) == "" {
		return Invalid("release_receipt_missing", "A live release for the verified Git revision and its deployment receipt are required to install the Feature.")
	}
	if strings.TrimSpace(liveRelease.DeploymentAttempt.LaunchURL) == "" || liveRelease.DeploymentAttempt.ResolvedAt == nil {
		return Invalid("release_launch_url_missing", "A live SaaS release requires a resolved public launch URL.")
	}
