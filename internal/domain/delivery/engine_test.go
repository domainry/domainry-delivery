...
func TestDeliveryRunFreezesOneFeatureRevisionAndGeneratesTests(t *testing.T) {
	...
	if len(run.TestCases) != len(run.Feature.Specification.Acceptance) || len(run.AcceptanceCases) != len(run.Feature.Specification.Acceptance) || len(run.WorkItems) != 0 {
		t.Fatalf("run derived the wrong execution objects: work=%d tests=%d uat=%d", len(run.WorkItems), len(run.TestCases), len(run.AcceptanceCases))
	}
}
