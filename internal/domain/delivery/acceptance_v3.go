...
const commandAcceptanceConfirm = "acceptance.confirm"
...
func acceptanceActions(run *DeliveryRun) []AvailableAction {
	...
	for _, acceptanceCase := range run.AcceptanceCases {
		...
		actions = append(actions, AvailableAction{
			Command: commandAcceptanceConfirm, TargetID: acceptanceCase.ID, ActorKind: ActorHuman,
		})
	}
	return actions
}
...
func confirmAcceptance(run *DeliveryRun, command Command, now time.Time) error {
	...
	if payload.Result == ResultFail {
		return routeV3Failure(run, acceptanceCase.FeatureID, payload.FailureOwner)
	}
	return nil
}
...
func allV3AcceptancePass(run *DeliveryRun, revision string) bool {
...
func acceptanceReleaseGate(run *DeliveryRun) ReleaseGate {
	...
	return ReleaseGate{
		...
		OK:     ready && allV3AcceptancePass(run, revision),
	}
}
