...
func TestProjectionForLocaleLocalizesDeliveryOwnedReleaseGates(t *testing.T) {
	run := DeliveryRun{
		...
		Issues:  []Issue{{ID: "issue-1", Status: IssueOpen}},
	}

	projection := ProjectionForLocale(run, "es-MX")
	actual := []ReleaseGate{
		projection.Workflow.ReleaseGates[0],
		projection.Workflow.ReleaseGates[3],
		projection.Workflow.ReleaseGates[4],
	}
	expected := []ReleaseGate{
		{Code: "feature_frozen", Label: "FeatureRevision fijada", Detail: "feature-1 / R1", OK: true},
		{Code: "technical_tests_passed", Label: "Pruebas técnicas superadas", Detail: "Sin build", OK: false},
		{Code: "issues_closed", Label: "Todos los Issues cerrados", Detail: "1 abiertos", OK: false},
	}
...
func TestEverySupportedNonEnglishLocaleHasCompleteDeliveryCopy(t *testing.T) {
	...
	for _, locale := range SupportedLocales[1:] {
		...
		if noBuildLabels[locale] == "" || openIssueFormats[locale] == "" {
			t.Fatalf("locale %q is missing release gate detail copy", locale)
		}
