...
func ProjectionForLocale(run DeliveryRun, locale string) Projection {
	...
	for index := range projection.Workflow.ReleaseGates {
		...
		if gate.Detail == "No build" {
			gate.Detail = noBuildLabels[locale]
		}
		if gate.Code == "issues_closed" {
			gate.Detail = fmt.Sprintf(openIssueFormats[locale], countOpenIssues(run))
		}
	}
	return projection
}
...
func countOpenIssues(run DeliveryRun) int {
	count := 0
...
var noBuildLabels = map[string]string{
	"zh": "暂无构建", "zh-hant": "暫無建置", "ja": "ビルドなし", "ko": "빌드 없음", "es": "Sin build", "pt": "Sem build", "fr": "Aucun build", "de": "Kein Build", "it": "Nessuna build", "tr": "Build yok", "ar": "لا يوجد بناء",
}

var openIssueFormats = map[string]string{
	"zh": "%d 个未关闭", "zh-hant": "%d 個未關閉", "ja": "%d 件未解決", "ko": "%d개 열림", "es": "%d abiertos", "pt": "%d abertos", "fr": "%d ouverts", "de": "%d offen", "it": "%d aperti", "tr": "%d açık", "ar": "%d مفتوحة",
}
