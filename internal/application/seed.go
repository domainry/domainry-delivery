...
func DemoProduct(now time.Time) delivery.Product {
	...
	for index, spec := range featureSpecs {
		...
		productRevision := uint64(index + 2)
		product.Features = append(product.Features, delivery.Feature{
			...
			Attachments: []delivery.FeatureAttachment{},
			Revisions: []delivery.FeatureRevision{{
				Number: featureRevision, Title: spec.title, Summary: spec.summary, Priority: "high", BaselineProductRevision: 4,
		...
		product.Revisions = append(product.Revisions, delivery.ProductRevision{
			Number: productRevision, Story: story, Definition: definition, Decisions: []delivery.ProductDecision{},
			SourceFeatureID: spec.id, FeatureRevision: featureRevision, CreatedBy: "product-owner", CreatedAt: now,
		})
	}
	return product
}
...
func DemoDeliveryRun(now time.Time) delivery.DeliveryRun {
	run, err := delivery.NewDeliveryRun(DemoProduct(now), "F-001", 1, delivery.DeliveryRunSpec{
	...
	}, delivery.Actor{ID: "rd-agent", Kind: delivery.ActorAgent}, now)
	if err != nil {
		panic(err)
	}
	return run
